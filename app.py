...
@st.cache_data
def load_data():
    # Try full file, fall back to sample
    ...
    for c in ['startup_name','sector','city_clean','investors_clean','stage','type']:
        if c in df.columns: df[c] = df[c].fillna("").astype(str).strip().str.title()
...
if sel_sector != "All": f = f[f['sector'] == sel_sector]
if sel_city   != "All": f = f[f['city_clean'] == sel_city]
...
ax.set_xlabel("Year"); ax.set_ylabel("Funding (USD)"); ax.set_title("Total Funding by Year")
st.pyplot(fig)
...
def hhi(x): return ((x/x.sum())**2).sum()
...
ax.set_title('Composite Risk Score (0–1)'); ax.set_xlabel('Year'); ax.set_ylabel('Risk Score'); ax.grid(True)
st.pyplot(fig)
